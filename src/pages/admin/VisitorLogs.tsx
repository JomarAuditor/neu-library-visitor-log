...
import {
  ...
  ClipboardList, QrCode, Mail, LogIn, LogOut, Timer, Globe,
} from 'lucide-react';
...
export default function VisitorLogs() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      exportCSV(all.map((l: VisitLog) => {
        const v = (l.visitors as any);
        return {
          Name:           v?.full_name                            ?? '',
          Email:          v?.email                               ?? '',
          'Student No.':  v?.student_number                      ?? '—',
          'Visitor Type': v?.visitor_type                        ?? '',
          College:        v?.programs?.colleges?.name ?? v?.colleges?.name ?? '—',
          Program:        v?.programs?.name                      ?? '—',
          Purpose:        l.purpose,
        ...
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex bg-neu-gray rounded-xl p-1 gap-0.5 border border-neu-border">
              {(['today','week','month','custom'] as Filter[]).map(f => (
                <button key={f} onClick={() => { setFilter(f); setPage(0); }}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                    filter === f ? 'bg-neu-blue text-white' : 'text-slate-500 hover:text-neu-blue hover:bg-neu-light'
                  }`}>
                  {f === 'today' ? 'Today' : f === 'week' ? 'Week' : f === 'month' ? 'Month' : 'Custom'}
                </button>
              ))}
            </div>
            {filter === 'custom' && (
            ...
            <button onClick={handleExport} disabled={exporting}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-neu-blue text-white text-xs font-semibold hover:bg-neu-mid transition-all disabled:opacity-60 shadow-sm">
              <Download size={13} />
      ...
      <div className="card animate-fade-up overflow-hidden">
        <div className="px-6 py-4 border-b border-neu-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-1 text-xs bg-neu-light text-neu-blue px-2 py-0.5 rounded-full font-semibold">{total.toLocaleString()}</span>
          </div>
          <p className="text-xs text-slate-400 hidden sm:block">Showing {logs.length} of {total.toLocaleString()}</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-neu-gray border-b border-neu-border">
                {['Visitor','College','Program','Purpose','Method','Date','Time In','Time Out','Duration'].map(h => (
            ...
            <tbody>
              {isLoading && [...Array(8)].map((_, i) => (
                <tr key={i} className="border-b border-neu-border/50">
                  {[...Array(9)].map((_, j) => <td key={j} className="px-4 py-3.5"><div className="h-3.5 bg-gray-100 rounded animate-pulse w-24" /></td>)}
                </tr>
              ))}
              ...
              {!isLoading && logs.map(log => {
                ...
                const college = v?.programs?.colleges?.name ?? v?.colleges?.name ?? '—';
                const program = v?.programs?.name           ?? '—';
                return (
                  <tr key={log.id} className="border-b border-neu-border/40 hover:bg-neu-gray/50 transition-colors">
                    ...
                    <td className="px-4 py-3.5"><p className="text-xs text-slate-600 max-w-[160px] leading-snug">{college}</p></td>
                    <td className="px-4 py-3.5"><p className="text-xs text-slate-600 max-w-[160px] leading-snug">{program}</p></td>
                    <td className="px-4 py-3.5">
                      <span className={PURPOSE_BADGE[log.purpose] ?? 'badge-gray'}>{log.purpose}</span>
                    </td>
                    <td className="px-4 py-3.5">
                      <span className="badge-gray gap-1">
                        {log.login_method === 'QR Code' && <><QrCode size={10} />QR</>}
                        {log.login_method === 'Email'   && <><Mail   size={10} />Email</>}
                        {log.login_method === 'Google'  && <><Globe  size={10} />Google</>}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      {log.time_out
                        ? <span className="badge-amber gap-1 whitespace-nowrap"><LogOut size={10} />{fmtTime(log.time_out)}</span>
                        : <span className="badge-blue gap-1"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />Inside</span>}
                    </td>
                    <td className="px-4 py-3.5">
                      {log.duration_minutes != null
                        ? <span className="badge-gray gap-1"><Timer size={10} />{fmtDuration(log.duration_minutes)}</span>
        ...
        {pages > 1 && (
          <div className="px-6 py-4 border-t border-neu-border flex items-center justify-between">
            <p className="text-xs text-slate-400">Page {page + 1} of {pages} &middot; {total.toLocaleString()} records</p>
