...
// =====================================================================
// FIXES:
//   - StatsCard uses prop "title" (not "label") -- matches its Props interface
//   - CollegeChart and CourseChart take "filter/from/to" props and fetch
//     their own data internally -- do NOT accept "data" prop
//   - AdminLayout wrapper removed -- layout handled by nested route in App.tsx
// =====================================================================

import { useState } from 'react';
import {
  Users, Calendar, CalendarDays, CalendarRange,
  Download, RefreshCw, Wifi, TrendingUp,
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const fromVal = filter === 'custom' ? cfrom : undefined;
  const toVal   = filter === 'custom' ? cto   : undefined;

  const handleExport = async () => {
    setExporting(true);
    try {
      const rows = await fetchAllLogsCSV(filter, fromVal, toVal);
      exportCSV(
        rows.map((l: VisitorLog) => ({
  ...
  return (
    <>
      <PageHeader title="Library Visitor Dashboard" subtitle="NEU Library Management System" />
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        {filter === 'custom' && (
          <div className="flex flex-wrap gap-2 items-center">
            <input
              type="date"
              className="input text-xs py-2 px-3 w-36"
              value={cfrom}
      ...
      {/* ── Stat cards ── */}
      {/* StatsCard prop is "title" not "label" -- matches the component's Props interface */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 mb-6">
        ...
        <div className="sm:col-span-2 xl:col-span-1 animate-fade-up">
          <div className="card-p bg-gradient-to-br from-neu-blue to-neu-mid border-0 relative overflow-hidden h-full min-h-[130px]">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/45 text-[10px] mt-0.5">Active sessions (no time out)</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── Charts ── */}
      {/* CollegeChart and CourseChart fetch their own data via hooks  */}
      {/* They accept: filter, from, to  --  NOT a "data" prop         */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        <CollegeChart filter={filter} from={fromVal} to={toVal} />
        <CourseChart  filter={filter} from={fromVal} to={toVal} />
      </div>
    </>
  );
}